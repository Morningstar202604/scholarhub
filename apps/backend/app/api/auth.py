...
async def login_two_factor(
    ...
    """Second step of 2FA login: pending token + TOTP/backup code → tokens.

    Accepts either a 6-digit TOTP code or a single-use backup code
    (``XXXXX-XXXXX``, generated by ``/auth/2fa/setup``). Verification
    uses the Fernet-encrypted secret on the user record (M2 path);
    legacy plaintext secrets were migrated to the encrypted column by
    migration 019.
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Two-factor session invalid; log in again",
        ) from None

    code = (payload.code or "").strip().replace(" ", "")
    if verify_totp(secret, code) is not None:
        return _issue_tokens(user, response)

    # Fall back to single-use backup codes (stored as SHA-256 hashes).
    import json as _json

    normalized = normalize_backup_code(code)
    try:
        stored = set(_json.loads(user.totp_backup_codes_hashed or "[]"))
    except (ValueError, TypeError):
        stored = set()
    candidate = hash_backup_code(normalized)
    if candidate in stored:
        stored.discard(candidate)
        user.totp_backup_codes_hashed = _json.dumps(sorted(stored))
        await db.commit()
        logger.info("two_factor_backup_code_used", user_id=user.id, remaining=len(stored))
        return _issue_tokens(user, response)
