...
[![License](https://img.shields.io/badge/license-PolyForm--NC-red.svg?style=flat-square)](LICENSE)
[![Python](https://img.shields.io/badge/python-3.12+-blue.svg?logo=python&logoColor=white&style=flat-square)](https://www.python.org/)
...
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-17-336791.svg?logo=postgresql&logoColor=white&style=flat-square)](https://www.postgresql.org/)
...
[![Modules](https://img.shields.io/badge/modules-10-blueviolet?style=flat-square)](#模块清单)
[![E2E](https://img.shields.io/badge/E2E_specs-56-22C55E?style=flat-square)](#测试)
[![Status](https://img.shields.io/badge/status-pre--alpha-orange?style=flat-square)](#项目状态)
...
**[阅读文档](#文档) · [快速开始](#快速开始) · [查看架构](#系统架构) · [参与贡献](#贡献) · [获取帮助](SUPPORT.md)**
...
ScholarHUB 是一套**开箱即用的学术期刊发表平台**,把"投稿 → 审稿 → 录用 → 发表 → 阅读 → 订阅"这条主流程从零做完整,让你不用再为每个期刊、研究室、会议从零写一套定制系统。它不是论文写作工具,也不是文献管理软件,而是一个真正能跑、能给作者投递、能给编辑审稿、能给读者浏览的网站。

适合的场景:

- 实验室 / 学院想自建一个内部预印本与正刊平台
- 会议组织方需要一套完整的稿件收集 + 同行评审系统
- 出版人想快速试水开放获取(OA)期刊的运营
- 教育、政府、慈善机构等需要非商业用途的发表平台
...
从一篇稿件的诞生到它被读者阅读,所有节点都在一个平台上闭环:

<div align="center">
<img src="docs/assets/workflow.svg" alt="投稿审稿主流程" width="900" />
</div>

1. 作者投递稿件,填好元数据(标题 / 摘要 / 学科 / 关键词 / DOI)
2. 编辑分配审稿人,审稿人接稿 / 拒稿
...
**模块注册表** — 启动时 `app.core.modules.load_all()` 按依赖顺序加载所有模块,把它们的 ORM 表注册到 `Base.metadata`、把它们的路由挂到 FastAPI app、把它们的 health 检查加入 `/health` 响应。新增模块只要在 `app/modules/` 下建目录并在 `load_all()` 注册即可,无需改动 core。
...
欢迎在 [GitCode 仓库](https://gitcode.com/badhope/scholarhub) 或
[GitHub 镜像](https://github.com/weed33834/scholarhub) 上提 issue 或 PR。
